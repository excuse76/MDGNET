...
class DC_and_CE_loss_3(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, out_aug: torch.Tensor, target: torch.Tensor):
        """
        ...
        dc_loss = self.dc(net_output, target_dice, loss_mask=mask) + self.dc(out_aug, target_dice, loss_mask=mask)\
            if self.weight_dice != 0 else 0
        ce_loss = self.ce(net_output, target[:, 0]) + self.ce(out_aug, target[:, 0])\
...
class CA_loss(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k_aug: torch.Tensor):
        # print('44444444444444')
        k_maps, q_maps = k_aug, q
...
class DC_and_CE_loss_ori(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, weight_ce=1, weight_dice=1, ignore_label=None,
                 dice_class=SoftDiceLoss):
        """
        Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        ...
        :param weight_dice:
        """
        super(DC_and_CE_loss_ori, self).__init__()
        if ignore_label is not None:
            ce_kwargs['ignore_index'] = ignore_label
