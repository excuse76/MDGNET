...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss,CA_loss
...
class nnUNetTrainer(object):
    ...
    # def mixup_data(self, x, y, d, alpha=1.0, device='cpu'):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            # data = self.mixup_data(data,target,[0,1,2])
            # print('data000',data.shape)
            output,q,out_aug,k_aug = self.network(data)
            # del data
            # l = self.loss(output, target)
            # l = self.loss(output, out_aug, target) + self.CA_loss(q,k_aug)
            l = self.loss(output, target) + self.loss(out_aug, target) + self.CA_loss(q,k_aug)
            # print('train---------------',self.loss(output, target), self.loss(out_aug, target) ,self.CA_loss(q,k_aug))
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # l = self.loss(output, out_aug, target) + self.CA_loss(q,k_aug)
            l = self.loss(output, target) + self.loss(out_aug, target) + self.CA_loss(q,k_aug)
            # print('val---------------',self.loss(output, out_aug, target),self.CA_loss(q,k_aug))
